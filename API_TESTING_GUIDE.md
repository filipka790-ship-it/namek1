...
1. **Сохраняйте tokens** - используйте их для всех запросов
2. **Следите за фазами** - разные действия доступны в разных фазах
3. **Проверяйте роли** - ведущий и судья имеют разные действия
4. **Используйте player_id** - обязательно передавайте корректный ID игрока
5. **Обновляйте состояние** - регулярно вызывайте `/state.php` для синхронизации
